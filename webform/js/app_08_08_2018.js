...
webform.controller('NECsCtrl', ['$scope', '$rootScope', '$location', 'dataRepository', '$timeout', '$translate','$anchorScroll','$window','$filter', function($scope, $rootScope, $location, dataRepository, $timeout, $translate,$anchorScroll,$window,$filter) { 
    ...
    //$scope.treeData.mandatoryPollutant = 0;
    ...
    $scope.emptyTreeDataClicked = {};
    $scope.emptyTreeDataClicked.id = 1;
    $scope.emptyTreeDataClicked.sectorsData = [];
    $scope.emptyTreeDataClicked.unionPolicyData = []; 
    $scope.emptyTreeDataClicked.pollutantsData = []; 
    $scope.emptyTreeDataClicked.entitiesData = [];
    $scope.emptyTreeDataClicked.mandatoryPollutant = 0;
    $scope.emptyTreeDataClicked.policyInstrumentsData = [];
    $scope.emptyTreeDataClicked.implementationStatusData = []; 
    $scope.emptyTreeDataClicked.projectionsScenarioData = []; 
    $scope.emptyTreeDataClicked.ammoniaEmissionsData = []; 
    $scope.emptyTreeDataClicked.ammoniumCarbonateFertilisersData = []; 
    $scope.emptyTreeDataClicked.ammoniaEmissionsLivestockData = []; 
    $scope.emptyTreeDataClicked.openFieldBurningData = []; 

    /*$scope.emptyTreeData = {};
    $scope.emptyTreeData.id = 1;
    $scope.emptyTreeData.sectorsData = require('./treeData/sectorsData.json');
    $scope.emptyTreeData.unionPolicyData = require('./treeData/unionPolicyData.json');
    $scope.emptyTreeData.pollutantsData = require('./treeData/pollutantsData.json');
    $scope.emptyTreeData.entitiesData = require('./treeData/entitiesData.json');
    //$scope.agricultureData = require('./agricultureData.json');
    $scope.emptyTreeData.mandatoryPollutant = 1;
    $scope.emptyTreeData.policyInstrumentsData = require('./treeData/policyInstrumentsData.json');
    $scope.emptyTreeData.implementationStatusData = require('./treeData/implementationStatusData.json');
    $scope.emptyTreeData.projectionsScenarioData = require('./treeData/projectionsScenarioData.json');
    $scope.emptyTreeData.ammoniaEmissionsData = require('./treeData/ammoniaEmissionsData.json');
    $scope.emptyTreeData.ammoniumCarbonateFertilisersData = require('./treeData/ammoniumCarbonateFertilisersData.json');
    $scope.emptyTreeData.ammoniaEmissionsLivestockData = require('./treeData/ammoniaEmissionsLivestockData.json');

    $scope.emptyTreeData.openFieldBurningData = require('./treeData/openFieldBurningData.json');*/

    //console.log('$scope.emptyTreeData');
    //console.log($scope.emptyTreeData);
    ...
    $scope.getTreeClicked = function(id) {
      $scope.current.clicked.id = id;
      $scope.current.clicked.sectorsTreeInstance = $scope.sectorsTreeInstance.jstree(true).get_selected();
      $scope.current.clicked.pollutantsTreeInstance = $scope.pollutantsTreeInstance.jstree(true).get_selected();
      $scope.current.clicked.unionPolicyTreeInstance = $scope.unionPolicyTreeInstance.jstree(true).get_selected();
      $scope.current.clicked.policyInstrumentsTreeInstance = $scope.policyInstrumentsTreeInstance.jstree(true).get_selected();
      $scope.current.clicked.implementationStatusTreeInstance = $scope.implementationStatusTreeInstance.jstree(true).get_selected();
      $scope.current.clicked.projectionsScenarioTreeInstance = $scope.projectionsScenarioTreeInstance.jstree(true).get_selected();
      $scope.current.clicked.ammoniaEmissionsTreeInstance = $scope.ammoniaEmissionsTreeInstance.jstree(true).get_selected();
      $scope.current.clicked.ammoniumCarbonateFertilisersTreeInstance = $scope.ammoniumCarbonateFertilisersTreeInstance.jstree(true).get_selected();
      $scope.current.clicked.ammoniaEmissionsLivestockTreeInstance = $scope.ammoniaEmissionsLivestockTreeInstance.jstree(true).get_selected();
      $scope.current.clicked.openFieldBurningTreeInstance = $scope.openFieldBurningTreeInstance.jstree(true).get_selected();
    }
    ...
    console.log($scope.instance.NEC_PAMs.NEC_PAM.length);
    console.log($scope.instance.NEC_PAMs.NEC_PAM);*/
    ...
    $scope.nodePolicyInstrumentsChanged = function(node, selected, event) {
      ...
      //$scope.current.treeData.policyInstrumentsData = $scope.policyInstrumentsTreeInstance.jstree(true).get_json();
      //$scope.policyInstrumentsTreeInstance.jstree(true).settings.core.data = $scope.current.treeData.policyInstrumentsData;
      $scope.current.treeData.policyInstrumentsData.Clicked = $scope.policyInstrumentsTreeInstance.jstree(true).get_selected();
      console.log('$scope.current.treeData.policyInstrumentsData.Clicked');
      console.log($scope.current.treeData.policyInstrumentsData.Clicked);
    }
    ...
    $scope.nodeImplementationStatusChanged = function(node, selected, event) {
      ...
      $scope.current.treeData.implementationStatusData = $scope.implementationStatusTreeInstance.jstree(true).get_json();
      $scope.implementationStatusTreeInstance.jstree(true).settings.core.data = $scope.current.treeData.implementationStatusData;
    }
    ...
    $scope.nodeProjectionsScenarioChanged = function(node, selected, event) {
      ...
      $scope.current.treeData.projectionsScenarioData = $scope.projectionsScenarioTreeInstance.jstree(true).get_json();
      $scope.projectionsScenarioTreeInstance.jstree(true).settings.core.data = $scope.current.treeData.projectionsScenarioData;
    }
    ...
    $scope.nodeAmmoniaEmissionsChanged = function(node, selected, event) {
      ...
      $scope.current.treeData.ammoniaEmissionsData = $scope.ammoniaEmissionsTreeInstance.jstree(true).get_json();
      $scope.ammoniaEmissionsTreeInstance.jstree(true).settings.core.data = $scope.current.treeData.ammoniaEmissionsData;
    }
    ...
    $scope.nodeAmmoniumCarbonateFertilisersChanged = function(node, selected, event) {
      ...
      $scope.current.treeData.ammoniumCarbonateFertilisersData = $scope.ammoniumCarbonateFertilisersTreeInstance.jstree(true).get_json();
      $scope.ammoniumCarbonateFertilisersTreeInstance.jstree(true).settings.core.data = $scope.current.treeData.ammoniumCarbonateFertilisersData;
    }
    ...
    $scope.nodeOpenFieldBurningChanged = function(node, selected, event) {
      ...
      $scope.current.treeData.openFieldBurningData = $scope.openFieldBurningTreeInstance.jstree(true).get_json();
      $scope.openFieldBurningTreeInstance.jstree(true).settings.core.data = $scope.current.treeData.openFieldBurningData;
    }
    ...
    $scope.nodeAmmoniaEmissionsLivestockChanged = function(node, selected, event) {
      ...
      $scope.ammoniaEmissionsLivestockTreeInstance.jstree(true).get_selected().forEach(function(node) {
        if ( $scope.ammoniaEmissionsLivestockTreeInstance.jstree(true).is_leaf(node) === true) {
          console.log(node + ": " + $scope.ammoniaEmissionsLivestockTreeInstance.jstree(true).is_leaf(node));
          $scope.current.PAM.Table1.Agriculture.AmmoniaEmissionsLivestock.push(node);
        }
      });

      $scope.current.treeData.ammoniaEmissionsLivestockData = $scope.ammoniaEmissionsLivestockTreeInstance.jstree(true).get_json();
      $scope.ammoniaEmissionsLivestockTreeInstance.jstree(true).settings.core.data = $scope.current.treeData.ammoniaEmissionsLivestockData;
    }
    ...
    $scope.nodeSectorChanged = function(node, selected, event) {
        ...
        $scope.current.treeData.sectorsData = $scope.sectorsTreeInstance.jstree(true).get_json();
        $scope.sectorsTreeInstance.jstree(true).settings.core.data = $scope.current.treeData.sectorsData;
    }
    ...
    $scope.nodePollutantChanged = function(node, selected, event) {
      ...
      console.log('nodePollutantChanged: $scope.current.treeDataClicked.mandatoryPollutant');
      console.log($scope.current.treeDataClicked.mandatoryPollutant);
      ...
      console.log('$scope.current.PAM.id');
      console.log($scope.current.PAM.id);

      $scope.current.treeDataClicked.pollutantsData = $scope.pollutantsTreeInstance.jstree(true).get_selected();
      console.log('$scope.current.treeDataClicked.pollutantsData');
      console.log($scope.current.treeDataClicked.pollutantsData);

      console.log('$scope.treeDataClicked');
      console.log($scope.treeDataClicked);

      //$scope.current.clicked = $scope.pollutantsTreeInstance.jstree(true).get_selected();

      /*console.log('$scope.current.treeData.pollutantsData pre');
      console.log($scope.current.treeData.pollutantsData);
      $scope.current.treeData.pollutantsData = $scope.pollutantsTreeInstance.jstree(true).get_json();
      console.log('$scope.current.treeData.pollutantsData post');
      console.log($scope.current.treeData.pollutantsData);

      $scope.pollutantsTreeInstance.jstree(true).settings.core.data = $scope.current.treeData.pollutantsData;*/
    }
    ...
    $scope.nodeUnionPolicyChanged = function(node, selected, event) {
      ...
      //$scope.current.treeData.unionPolicyData = $scope.unionPolicyTreeInstance.jstree(true).get_json();
      //$scope.unionPolicyTreeInstance.jstree(true).settings.core.data = $scope.current.treeData.unionPolicyData;

      $scope.current.clicked = $scope.unionPolicyTreeInstance.jstree(true).get_selected();
  }
    ...
    dataRepository.getInstance()
            ...
            if (!angular.isDefined(instance)) { return; }
              ...
              for (var i = 0;i < instance.NEC_PAMs.NEC_PAM.length;i++) {
                ...
                normalizeArray(instance.NEC_PAMs.NEC_PAM[i].Table1,"PolicyInstrument");
                ...
                //normalizeArray(instance.NEC_PAMs.NEC_PAM[i].Table2,"PolicyImpact");
                //normalizeArray(instance.NEC_PAMs.NEC_PAM[i].Table2,"ExanteDocumentation");
                //normalizeArray(instance.NEC_PAMs.NEC_PAM[i].Table2,"Expost");          
                //normalizeArray(instance.NEC_PAMs.NEC_PAM[i].Table2,"ExpostDocumentation");
  ...
  $scope.isEmptyT = function(index) {
    if ($scope.isNullEmpty($scope.instance.NEC_PAMs.NEC_PAM[index].Title)) { return true; }
    return false;
  };
  //NOT USED END!
  ...
  $scope.setCurrentTreeData = function() {
    ...
    $scope.current.treeDataClicked.pollutantsData.forEach (function(node) {
      $scope.pollutantsTreeInstance.jstree(true).select_node (node , true, false);
    })
    ...
    $scope.emptyTreeDataClicked.pollutantsData = []; 
    $scope.emptyTreeDataClicked.entitiesData = [];
    $scope.emptyTreeDataClicked.mandatoryPollutant = 1;
    $scope.emptyTreeDataClicked.policyInstrumentsData = [];
    $scope.emptyTreeDataClicked.implementationStatusData = []; 
    $scope.emptyTreeDataClicked.projectionsScenarioData = []; 
    $scope.emptyTreeDataClicked.ammoniaEmissionsData = []; 
    ...
    */
    ...
    /*$scope.sectorsTreeInstance.jstree(true).settings.core.data = treeData.sectorsData;
    $scope.sectorsTreeInstance.jstree(true).refresh(1, true);

    console.log('treeData.pollutantsData');
    console.log(treeData.pollutantsData);
    $scope.pollutantsTreeInstance.jstree(true).settings.core.data = treeData.pollutantsData;
    $scope.pollutantsTreeInstance.jstree(true).refresh(1, true);

    $scope.unionPolicyTreeInstance.jstree(true).settings.core.data = treeData.unionPolicyData;
    $scope.unionPolicyTreeInstance.jstree(true).refresh(1, true);*/
    
    /*if ($scope.current.treeData.policyInstrumentsData.Clicked) {
      $scope.policyInstrumentsTreeInstance.jstree(true).deselect_all();
      $scope.current.treeData.policyInstrumentsData.Clicked.forEach (function(node) {
        $scope.policyInstrumentsTreeInstance.jstree(true).select_node (node , true, false);
        console.log(node);
      });
    }*/

    //$scope.policyInstrumentsTreeInstance.jstree(true).settings.core.data = treeData.policyInstrumentsData;
    //$scope.policyInstrumentsTreeInstance.jstree(true).refresh(1, true);
    
    /*$scope.implementationStatusTreeInstance.jstree(true).settings.core.data = treeData.implementationStatusData;
    $scope.implementationStatusTreeInstance.jstree(true).refresh(1, true);
    
    $scope.projectionsScenarioTreeInstance.jstree(true).settings.core.data = treeData.projectionsScenarioData;
    $scope.projectionsScenarioTreeInstance.jstree(true).refresh(1, true);
    
    $scope.ammoniaEmissionsTreeInstance.jstree(true).settings.core.data = treeData.ammoniaEmissionsData;
    $scope.ammoniaEmissionsTreeInstance.jstree(true).refresh(1, true);
    
    $scope.ammoniumCarbonateFertilisersTreeInstance.jstree(true).settings.core.data = treeData.ammoniumCarbonateFertilisersData;
    $scope.ammoniumCarbonateFertilisersTreeInstance.jstree(true).refresh(1, true);
    
    $scope.ammoniaEmissionsLivestockTreeInstance.jstree(true).settings.core.data = treeData.ammoniaEmissionsLivestockData;
    $scope.ammoniaEmissionsLivestockTreeInstance.jstree(true).refresh(1, true);
    
    $scope.openFieldBurningTreeInstance.jstree(true).settings.core.data = treeData.openFieldBurningData;
    $scope.openFieldBurningTreeInstance.jstree(true).refresh(1, true);*/
  };
  ...
  $scope.setCurrentPam = function(internalId) {        
    ...
    for (var i = 0; i < $scope.instance.NEC_PAMs.NEC_PAM.length; i++) {
      if ($scope.instance.NEC_PAMs.NEC_PAM[i].internalId == internalId) {
        ...
        //$scope.current.treeData = $scope.treeData[i+1];
        $scope.current.treeDataClicked = $scope.treeDataClicked[i+1];
        console.log('$scope.current.treeDataClicked');
        console.log($scope.current.treeDataClicked);
  ...
  $scope.Group.getPolicyInstrument = function() {
    ...
    $scope.current.PAM.Table1.PolicyInstruments = tmpPolicyInstrument;
  };
...
//  }
  ...
  $scope.addSingle = function(goEdit) {
    ...
    //var clonedEmptyTreeData = angular.copy($scope.emptyTreeData);
    var clonedEmptyTreeDataClicked = angular.copy($scope.emptyTreeDataClicked);

    clonedEmptyPaM.id = $scope.getNextId();
    //clonedEmptyTreeData.id = clonedEmptyPaM.id;
    clonedEmptyTreeDataClicked.id = clonedEmptyPaM.id;

    $scope.setRadioButtons(clonedEmptyPaM);

    $scope.instance.NEC_PAMs.NEC_PAM.push(clonedEmptyPaM);
    //$scope.treeData.push(clonedEmptyTreeData);
    $scope.treeDataClicked.push(clonedEmptyTreeDataClicked);
  ...
  $scope.deletePAM = function(internalId) {
    if (confirm('Are you sure you want to delete this PAM')) {
      ...
      $scope.treeDataClicked.splice($scope.getIndexForInternalId(internalId),1);
