...
webform.controller('NECsCtrl', ['$scope', '$rootScope', '$location', 'dataRepository', '$timeout', '$translate','$anchorScroll','$window','$filter', function($scope, $rootScope, $location, dataRepository, $timeout, $translate,$anchorScroll,$window,$filter) { 
    ...
    $scope.treeConfig = {
        ...
        plugins: ['checkbox'],
        checkbox: { whole_node: true,  hide_icons: true}
    };

    $scope.checkStatus = function(node, selected, event) {
        var currentNode = $scope.treeInstance.jstree("get_selected");
    }
    ...
    $scope.nodeSectorChanged = function(node, selected, event) {
        ...
        nodes.forEach(function(node) {
          sectors.push(node.substring(0, 2));
          if (node.length > 2) { ojectives.push(node); }
        });
        $scope.current.PAM.Table1.Sectors = $.unique(sectors);
        $scope.current.PAM.Table1.Ojectives = $.unique(ojectives);
    }
    ...
    $scope.nodeUnionPolicyChanged = function(node, selected, event) {
      ...
      var unionPolicies = [];
      nodes.forEach(function(node) {
        unionPolicies.push(node);
      });

      //if ($.inArray( "Other_EU", unionPolicies ) == -1) {
        //console.log($scope.current.PAM.Table1.UnionPolicyOther);
        //$scope.current.PAM.Table1.UnionPolicyOther.Name = ""; 
        //console.log($scope.current.PAM.Table1.UnionPolicyOther);
        //$scope.addItem('current.Table1.UnionPolicyOther');
      //}
      $scope.UpdateOtherPolicies();

      $scope.current.PAM.Table1.UnionPolicy = $.unique(unionPolicies);
    ...
    dataRepository.getInstance()
            ...
            //Modify Object to use arrays properly     
              ...
              for (var i = 0;i < instance.MMR_PAMs.MMR_PAM.length;i++) {
                ...
                normalizeArray(instance.MMR_PAMs.MMR_PAM[i].Table1,"GreenhouseGases");
  ...
  $scope.hasObjective = function(obj) {
    ...
    if (objectives.indexOf(obj) !== -1) { return true; }
    return false;
  };
  ...
  //Function to get Grouped GreenhouseGases
  $scope.Group.getGreenhouseGases = function() {
    var tempGroup = $scope.current.PAM.Table1.PolicyGroup;
    var tmpGases = [];
    for (var i = 0;i < tempGroup.length;i++) {
      var tmp = $filter('filter')($scope.instance.MMR_PAMs.MMR_PAM, {id: tempGroup[i]}, true)[0].Table1;
      if (angular.isDefined(tmp.GreenhouseGases)) {
        for (var j = 0;j < tmp.GreenhouseGases.length;j++) {            
          if (tmpGases.indexOf(tmp.GreenhouseGases[j]) === -1) {
            tmpGases.push(tmp.GreenhouseGases[j]);
          }
        }
      }        
    }      
    $scope.current.PAM.Table1.GreenhouseGases = tmpGases;
  };
  ...
  //Function to get Envisaged value for individual PAMs
  $scope.Group.getEnvisaged = function() {
    var tempGroup = $scope.current.PAM.Table1.PolicyGroup;
    var isEnvisaged = 'no';
    for (var i = 0; i < tempGroup.length;i++) {
      var tmp = $filter('filter')($scope.instance.MMR_PAMs.MMR_PAM, {id: tempGroup[i]}, true)[0].Table1;
      if (angular.isDefined(tmp.isEnvisaged) && tmp.isEnvisaged === 'yes') {
        isEnvisaged = tmp.isEnvisaged;
      }
    }
    $scope.current.PAM.Table1.isEnvisaged = isEnvisaged;
  };
  ...
  $scope.getGroupList = function(single) {    
    ...
    $scope.OverviewValidate = function() {
      ...
      for (var i = 0; i < tmp.length;i++) { 
        ...
        if ($scope.isNullEmpty(PAM.Title) || $scope.isNullEmpty(PAM.Table1.Description) || $scope.isNull(PAM.Table1.isGroup)
                || $scope.isNullEmpty(PAM.Table1.Sectors) || $scope.isNullEmpty(PAM.Table1.GreenhouseGases) || $scope.isNullEmpty(PAM.Table1.Objective)
                || $scope.isNull(PAM.Table1.isEnvisaged) || $scope.isNullEmpty(PAM.Table1.PolicyInstrument) || $scope.isNullEmpty(PAM.Table1.Implementation[0].Status)
    ...
    $scope.Group.calculateGroup = function() {  
      ...
      $scope.Group.getGreenhouseGases();
