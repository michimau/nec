...
webform.controller('NECsCtrl', ['$scope', '$rootScope', '$location', 'dataRepository', '$timeout', '$translate','$anchorScroll','$window','$filter', function($scope, $rootScope, $location, dataRepository, $timeout, $translate,$anchorScroll,$window,$filter) { 
    ...
    $scope.nodePolicyInstrumentsChanged = function(node, selected, event) {
      ...
      $scope.current.treeDataClicked.policyInstrumentsData = $scope.policyInstrumentsTreeInstance.jstree(true).get_selected();
    }
    ...
    $scope.nodeImplementationStatusChanged = function(node, selected, event) {
      ...
      $scope.current.treeDataClicked.implementationStatusData = $scope.implementationStatusTreeInstance.jstree(true).get_selected();
    }
    ...
    $scope.nodeProjectionsScenarioChanged = function(node, selected, event) {
      ...
      $scope.current.treeDataClicked.projectionsScenarioData = $scope.projectionsScenarioTreeInstance.jstree(true).get_selected();
    }
    ...
    $scope.nodeAmmoniaEmissionsChanged = function(node, selected, event) {
      ...
      $scope.current.treeDataClicked.ammoniaEmissionsData = $scope.ammoniaEmissionsTreeInstance.jstree(true).get_selected();
    }
    ...
    $scope.nodeAmmoniumCarbonateFertilisersChanged = function(node, selected, event) {
      ...
      $scope.current.treeDataClicked.ammoniumCarbonateFertilisersData = $scope.ammoniumCarbonateFertilisersTreeInstance.jstree(true).get_selected();
    }
    ...
    $scope.nodeOpenFieldBurningChanged = function(node, selected, event) {
      ...
      $scope.current.treeDataClicked.openFieldBurningData = $scope.openFieldBurningTreeInstance.jstree(true).get_selected();
    }
    ...
    $scope.nodeAmmoniaEmissionsLivestockChanged = function(node, selected, event) {
      ...
      $scope.current.treeDataClicked.ammoniaEmissionsLivestockData = $scope.ammoniaEmissionsLivestockTreeInstance.jstree(true).get_selected();
    }
    ...
    $scope.nodeSectorChanged = function(node, selected, event) {
        ...
        $scope.current.treeDataClicked.sectorsData = $scope.sectorsTreeInstance.jstree(true).get_selected();
    }
    ...
    $scope.nodeUnionPolicyChanged = function(node, selected, event) {
      ...
      $scope.current.treeDataClicked.unionPolicyData = $scope.unionPolicyTreeInstance.jstree(true).get_selected();
  }
    ...
    var normalizeArray = function(obj, propertyName) {
      ...
      if (angular.isArray(propertyValue)) {
          console.log('property isArray');
          return;
      }        
      if (angular.isUndefined(propertyValue) || propertyValue === null) {
          obj[propertyName] = [];
          console.log('property isUndefined');
          return;
      }        
      var arrayValue = [];
  ...
  $scope.setCurrentTreeData = function() {
    ...
    $scope.current.treeDataClicked.sectorsData.forEach (function(node) {
      $scope.sectorsTreeInstance.jstree(true).select_node (node , true, false);
    })

    $scope.policyInstrumentsTreeInstance.jstree(true).deselect_all(true);
    $scope.current.treeDataClicked.policyInstrumentsData.forEach (function(node) {
    ...
    $scope.current.treeDataClicked.implementationStatusData.forEach (function(node) {
      $scope.implementationStatusTreeInstance.jstree(true).select_node (node , true, false);
    })

    $scope.unionPolicyTreeInstance.jstree(true).deselect_all(true);
    $scope.current.treeDataClicked.unionPolicyData.forEach (function(node) {
      $scope.unionPolicyTreeInstance.jstree(true).select_node (node , true, false);
    })

    $scope.projectionsScenarioTreeInstance.jstree(true).deselect_all(true);
    $scope.current.treeDataClicked.projectionsScenarioData.forEach (function(node) {
    ...
    $scope.current.treeDataClicked.ammoniaEmissionsData.forEach (function(node) {
      $scope.ammoniaEmissionsTreeInstance.jstree(true).select_node (node , true, false);
    })

    $scope.ammoniumCarbonateFertilisersTreeInstance.jstree(true).deselect_all(true);
    $scope.current.treeDataClicked.ammoniumCarbonateFertilisersData.forEach (function(node) {
      $scope.ammoniumCarbonateFertilisersTreeInstance.jstree(true).select_node (node , true, false);
    })

    $scope.ammoniaEmissionsLivestockTreeInstance.jstree(true).deselect_all(true);
    $scope.current.treeDataClicked.ammoniaEmissionsLivestockData.forEach (function(node) {
      $scope.ammoniaEmissionsLivestockTreeInstance.jstree(true).select_node (node , true, false);
    })

    $scope.openFieldBurningTreeInstance.jstree(true).deselect_all(true);
    $scope.current.treeDataClicked.openFieldBurningData.forEach (function(node) {
      $scope.openFieldBurningTreeInstance.jstree(true).select_node (node , true, false);
    })
  };
  ...
  $scope.setCurrentPam = function(id) {
    console.log('setCurrentPam id');
    console.log(id);  
    if (!$scope.instance.NEC_PAMs.NEC_PAM) { return; }
  
    /*$scope.current.PAM = $scope.instance.NEC_PAMs.NEC_PAM[id-1];
    $scope.current.treeDataClicked = $scope.treeDataClicked[id-1];*/
    ...
    for (var i = 0; i < $scope.instance.NEC_PAMs.NEC_PAM.length; i++) {
      if ($scope.instance.NEC_PAMs.NEC_PAM[i].internalId == id) {
        ...
        console.log('setCurrentPAM $scope.current.treeDataClicked');
...
/*  $scope.sectorsFind = function(item) {      
  ...
  $scope.getGroupList = function(single) {    
    ...
    $scope.saveInstance = function(){
      ...
      console.log('saveinstance tmpScope.NEC_PAMs.NEC_PAM');
      console.log(tmpScope.NEC_PAMs.NEC_PAM);
